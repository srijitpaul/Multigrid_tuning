...
sed '5s/.*/#SBATCH -N '$nodes'/' mg_knl.sh > $filelocation/node.sh
...
sed '13s#.*#srun -n '$cores'  ./qlua_avx2_cori  plaquette.qlua load_gauge_field.qlua stout_smear.qlua '$qlua_filename'.qlua #' $filelocation/err.sh > $filelocation/${qlua_filename}_${filelocation}_${cores}.sh
